...
/**
 * https://htx.htxcct.vip/Htx_888.php 接口
 * 与迪拜第三方对接的具体实现
 */
public class DuBaiInterface extends CommonPayInterface {
    ...
    public String onNotify(HttpServletRequest request) {
        ...
        try{
            ...
            if (sign.equals(notifySign)) {
                ...
                params.put("sign",getSign(params,token).toLowerCase());
                String result = HttpUtil.doPOST("https://htx.htxcct.vip/?c=Pay&a=query", params);
                logger.info("迪拜:查询订单结果返回的JSON："+result);
