 ...
 * 与第三方对接的具体实现
 */
public class YiDao2Interface extends CommonPayInterface {
    ...
    public ApiResult onPay(HttpServletRequest request, PayOrder payOrder, PayChannel payChannel) {
        ...
        if(!StringUtils.isEmpty(result)){
            ...
            if(jsonObject.getIntValue("code")==200){
                ...
                if(payChannel.getPayInterfaceType().equalsIgnoreCase("6")||payChannel.getPayInterfaceType().equalsIgnoreCase("7")){
                    return new ApiResult().ok("订单创建成功").inject(data.getString("request_url"));
                }else{
//                    String data=jsonObject.getString("data");
//                    redisComponent.set(RedisConstant.getOrderInfo(payOrder.getOrderNo()),data,60*30);
