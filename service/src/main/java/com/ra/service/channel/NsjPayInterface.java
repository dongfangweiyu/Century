...
/**
 * https://biz.nsjpay.com/     接口
 * 与新世纪支付第三方对接的具体实现
 */
public class NsjPayInterface extends CommonPayInterface {
    ...
    public ApiResult onPay(HttpServletRequest request, PayOrder payOrder, PayChannel payChannel) {
        ...
        if(banktype.equalsIgnoreCase("ALIPAY_H5")){
            params.put("ish5",true);
            banktype="ALIPAY";
        }else{
            params.put("ish5",false);
        }
        ...
        logger.info("NsjPayInterface:创建订单返回JSON："+result);
    ...
    private String generalCreateSign(String partner,String banktype,BigDecimal paymoney,
        ...
        sb.append(key);
        logger.info("NsjPayInterface签名加密源串："+sb.toString());
        String md5Value = EncryptUtil.encodeMD5(sb.toString()).toLowerCase();
        logger.info("NsjPayInterface签名加密结果："+md5Value);
        return md5Value;
    }
