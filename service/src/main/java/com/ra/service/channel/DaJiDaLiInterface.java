 ...
 * 与第三方对接的具体实现
 */
public class DaJiDaLiInterface extends CommonPayInterface {
    ...
    public ApiResult onPay(HttpServletRequest request, PayOrder payOrder, PayChannel payChannel) {
        ...
        params.put("out_order_sn", payOrder.getOrderNo());
        params.put("timestamp", timestamp);
        params.put("client_ip", "127.0.0.1");
        params.put("order_desc", "");
        params.put("nonce_string",nonceStr);//业务流水号，18位长度以上的随机字符串
        params.put("orderDesc", payOrder.getOrderDesc());// 商品信息描述
        params.put("signature", generateCreateSign( payOrder.getAmount().toPlainString(),payOrder.getOrderNo(),appId,timestamp.toString()
                ,nonceStr, payChannel.getPayInterfaceType(), secret));// 签名
       String result = HttpUtil.doPOST(config.getCreateOrderUrl(), params);
        logger.info("DaJiDaLi:创建订单返回JSON："+result);
    ...
    public String onNotify(HttpServletRequest request) {
        logger.info("收到DaJiDaLi支付结果的异步通知...");
        ...
        try{
            // 延签
            // 验证是不是支付网关发来的异步通知
            String sign=generateCreateSign(amount,out_order_sn,appId,timestamp,nonce_string,pay_code,serect );
     ...
     * @return 回调参数签名
     */
    private static String generateCreateSign( String amount,String outOrderNo, String appId,
                                              String timestamp, String nonceString ,String payCode,String secret) {

        Map<String,String> params=new HashMap<>();
        params.put("amount",amount);
        params.put("out_order_sn",outOrderNo);
