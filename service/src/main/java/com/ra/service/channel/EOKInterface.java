...
/**
 * http://eok.ydfcpf.top/Home_Index_document.html#doc22 接口
 * 与EOK第三方对接的具体实现
 */
public class EOKInterface extends CommonPayInterface {
    ...
    public ApiResult onPay(HttpServletRequest request, PayOrder payOrder, PayChannel payChannel) {
        ...
        logger.info("EOKInterface:创建订单返回JSON："+result);
        //{"status":"error","msg":"用户未分配通道,暂时无法连接!","data":[]}
        if(!StringUtils.isEmpty(result)){
            JSONObject jsonObject = JSON.parseObject(result);
            if(jsonObject.getString("status").equalsIgnoreCase("success")){
                jsonObject=jsonObject.getJSONObject("data");
                return new ApiResult().ok("订单创建成功").inject(jsonObject.getString("pay_callbackurl"));
            }else{
    ...
    public String onNotify(HttpServletRequest request) {
        ...
        if(payOrder.getAmount().compareTo(BigDecimal.valueOf(Integer.parseInt(amount)))!=0){
            return "金额不对";
        }
